...
class Settings(BaseSettings):
    ...
    PORT: int = Field(default=9111, ge=1, le=65535, description="Port du serveur")
    ...
    # --- Sécurité & JWT ---
    # Valeur par défaut pour exe Windows sans .env ; en production, définir SECRET_KEY dans .env
    SECRET_KEY: str = Field(
        default="gesco-dev-default-change-me-in-production-32c",
        min_length=32,
    ...
    SYNC_BATCH_SIZE: int = Field(default=100, ge=1, le=10_000, description="Taille des lots pour la synchro")
    SYNC_OFFLINE_THRESHOLD_SECONDS: int = Field(default=300, ge=0, description="Seuil (secondes) pour considérer une session hors ligne")

    # --- Seed (premier démarrage) ---
    SEED_SUPERUSER_EMAIL: str | None = Field(default=None, description="Email du superutilisateur à créer au seed (optionnel)")
    SEED_SUPERUSER_PASSWORD: str | None = Field(default=None, description="Mot de passe du superutilisateur au seed (optionnel)")

    # --- SMTP (notifications / emails) ---
    SMTP_HOST: str | None = Field(default=None, description="Serveur SMTP")
    ...
    SMTP_FROM: str = Field(default="noreply@gesco.local", description="Adresse expéditrice par défaut")
    SMTP_TLS: bool = Field(default=True, description="Utiliser TLS pour SMTP")

    # --- Sentry (monitoring) ---
    SENTRY_DSN: str | None = Field(default=None, description="DSN Sentry (vide = désactivé)")
    SENTRY_ENVIRONMENT: str = Field(default="development", description="Environnement Sentry")
    SENTRY_TRACES_SAMPLE_RATE: float = Field(default=0.1, ge=0.0, le=1.0, description="Taux d'échantillonnage des traces")

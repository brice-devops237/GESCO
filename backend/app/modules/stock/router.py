...
# -----------------------------------------------------------------------------
# Routes API v1 pour le module Stock. Isolation multi-tenant : accès via dépôt
# ou produit (entreprise_id) ; list_mouvements et list_alertes scopés par depot_id
# validé. Extension monde réel : toutes structures, tous secteurs.
# -----------------------------------------------------------------------------
...
def _check_depot_entreprise(depot, current_user):
    if depot is None:
        raise NotFoundError(detail="Dépôt non trouvé.")
...
def _check_produit_entreprise(produit, current_user):
    if produit is None:
        raise NotFoundError(detail="Produit non trouvé.")
    if produit.entreprise_id != current_user.entreprise_id:
        raise ForbiddenError(detail="Accès à une autre entreprise non autorisé", code="FORBIDDEN_ENTREPRISE")
...
async def list_stocks_by_depot(
    ...
    depot = await DepotRepository(db).find_by_id(depot_id)
    _check_depot_entreprise(depot, current_user)
...
async def get_quantite_stock(
    ...
    depot = await DepotRepository(db).find_by_id(depot_id)
    _check_depot_entreprise(depot, current_user)
    produit = await ProduitRepository(db).find_by_id(produit_id)
    _check_produit_entreprise(produit, current_user)
...
async def get_mouvement(db: DbSession, current_user: CurrentUser, id: int):
    ent = await MouvementService(db).get_or_404(id)
    depot = await DepotRepository(db).find_by_id(ent.depot_id)
    _check_depot_entreprise(depot, current_user)
    return ent
...
async def create_mouvement(
    ...
    depot = await DepotRepository(db).find_by_id(data.depot_id)
    _check_depot_entreprise(depot, current_user)
    produit = await ProduitRepository(db).find_by_id(data.produit_id)
    _check_produit_entreprise(produit, current_user)
    if data.depot_dest_id is not None:
        depot_dest = await DepotRepository(db).find_by_id(data.depot_dest_id)
        _check_depot_entreprise(depot_dest, current_user)

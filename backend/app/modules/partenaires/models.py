...
# -----------------------------------------------------------------------------
# Modèles ORM du module Partenaires : types de tiers, tiers (clients/fournisseurs),
# contacts. Dépend de Paramétrage (entreprises), Catalogue (canaux_vente).
...
# --- Type de tiers (référentiel) ----------------------------------------------
class TypeTiers(Base):
    """Type de tiers (client, fournisseur, etc.). Table : types_tiers."""
...
# Réalité terrain : identification légale (NIU, RCCM), adresse postale (BP courante en Afrique).
class Tiers(Base):
    """Tiers : client ou fournisseur. Table : tiers. Soft delete."""
    __tablename__ = "tiers"
    ...
    boite_postale: Mapped[str | None] = mapped_column(String(20), nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint("entreprise_id", "code", name="uq_tiers_entreprise_code"),
    )


# --- Contact (par tiers) ------------------------------------------------------
# Civilité pour courriers et factures (M., Mme, Mlle, etc.).
class Contact(Base):
    """Contact d'un tiers. Table : contacts."""
    __tablename__ = "contacts"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    tiers_id: Mapped[int] = mapped_column(Integer, ForeignKey("tiers.id"), nullable=False)
    civilite: Mapped[str | None] = mapped_column(String(10), nullable=True)  # M., Mme, Mlle, Dr., etc.

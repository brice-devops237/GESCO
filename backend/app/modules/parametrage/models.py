...
# Référence : CGI Cameroun, OHADA (formes juridiques), pratiques CEMAC/UEMOA.
class RegimeFiscal(str, PyEnum):
    """Régime fiscal (CGI Cameroun, notions réutilisables en zone CEMAC/OHADA)."""
    informel = "informel"
...
class FormeJuridique(str, PyEnum):
    """Forme juridique (OHADA / droit des sociétés – Cameroun, Sénégal, Côte d'Ivoire, etc.)."""
...
# --- Taux de change ----------------------------------------------------------
# En pratique : BCEAC (CEMAC), banques centrales, ou taux manuels/API par date.
class TauxChange(Base):
    ...
    source: Mapped[str | None] = mapped_column(String(30), nullable=True)  # bcceac, banque_centrale, manuel, api
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)


# --- Entreprise --------------------------------------------------------------
# Représentation réelle : identification légale (NIU, RCCM, CNPS), forme juridique,
# adresse postale (boîte postale courante en Afrique), fuseau, capital (OHADA).
class Entreprise(Base):
    ...
    forme_juridique: Mapped[str | None] = mapped_column(String(30), nullable=True)  # OHADA : SARL, SA, etc.
    rccm: Mapped[str | None] = mapped_column(String(50), nullable=True)   # Registre du Commerce (OHADA)
    cnps: Mapped[str | None] = mapped_column(String(30), nullable=True)  # Numéro CNPS / sécurité sociale
    ...
    boite_postale: Mapped[str | None] = mapped_column(String(20), nullable=True)  # Courant en Afrique
    ...
    fuseau_horaire: Mapped[str | None] = mapped_column(String(50), nullable=True)  # IANA ex. Africa/Douala
    ...
    date_creation: Mapped[date | None] = mapped_column(Date, nullable=True)  # Date de création juridique
    capital_social: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), nullable=True)  # Montant (devise principale)
...
# --- Point de vente ----------------------------------------------------------
# Réalité terrain : adresse complète, code postal, fuseau pour horaires/rapports.
class PointDeVente(Base):
    ...
    ville: Mapped[str | None] = mapped_column(String(100), nullable=True)
    telephone: Mapped[str | None] = mapped_column(String(30), nullable=True)
    latitude: Mapped[Decimal | None] = mapped_column(Numeric(10, 7), nullable=True)   # WGS84, affichage carte
    longitude: Mapped[Decimal | None] = mapped_column(Numeric(11, 7), nullable=True)  # WGS84, affichage carte
    fuseau_horaire: Mapped[str | None] = mapped_column(String(50), nullable=True)  # Hérite entreprise si vide

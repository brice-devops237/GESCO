...
# Conçu pour toute structure (PME à grand groupe) et tout secteur (commerce,
# industrie, services).
# -----------------------------------------------------------------------------
...
# --- Unité de mesure ---------------------------------------------------------
# Référence : UN/CEFACT Recommendation 20 (codes internationaux : KGM, LTR, PCE, MTR…).
class UniteMesure(Base):
    ...
    code_cefact: Mapped[str | None] = mapped_column(String(10), nullable=True)  # UN/CEFACT Rec 20 (KGM, LTR, PCE)
    actif: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)


# --- Taux TVA (CGI Cameroun, CEMAC) ------------------------------------------
# nature : normal, reduit, exonere (reporting et affichage factures, loi de finances).
class NatureTva(str, PyEnum):
    """Nature du taux TVA pour reporting et mentions factures (CGI, CEMAC)."""
    normal = "normal"
    reduit = "reduit"
    exonere = "exonere"
...
class TauxTva(Base):
    ...
    nature: Mapped[str | None] = mapped_column(String(20), nullable=True)  # normal, reduit, exonere
    actif: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
...
# --- Conditionnement ---------------------------------------------------------
# Poids net, type d'emballage : logistique, transport, douane (CEMAC).
class TypeEmballage(str, PyEnum):
    """Type d'emballage courant (logistique, export)."""
    caisse = "caisse"
    carton = "carton"
    palette = "palette"
    sachet = "sachet"
...
class Conditionnement(Base):
    ...
    type_emballage: Mapped[str | None] = mapped_column(String(30), nullable=True)  # caisse, carton, palette…
    poids_net_kg: Mapped[Decimal | None] = mapped_column(Numeric(12, 3), nullable=True)
    actif: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
...
# --- Produit -----------------------------------------------------------------
# Référence fournisseur, marque, code douanier (CEMAC/HS), pays d'origine, poids (logistique/douane).
class Produit(Base):
    ...
    reference_fournisseur: Mapped[str | None] = mapped_column(String(80), nullable=True)
    code_douanier: Mapped[str | None] = mapped_column(String(20), nullable=True)  # Nomenclature douanière CEMAC/HS
    pays_origine: Mapped[str | None] = mapped_column(String(3), nullable=True)  # ISO 3166-1 alpha-3 (douane)
    poids_net_kg: Mapped[Decimal | None] = mapped_column(Numeric(12, 3), nullable=True)

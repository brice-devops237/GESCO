# app/modules/rh/services/departement.py
# -----------------------------------------------------------------------------
# Service métier : départements (services).
# -----------------------------------------------------------------------------
...
class DepartementService(BaseRHService):
    def __init__(self, db: AsyncSession) -> None:
        super().__init__(db)
        self._repo = DepartementRepository(db)
        self._entreprise_repo = EntrepriseRepository(db)

    async def get_by_id(self, id: int) -> Departement | None:
        return await self._repo.find_by_id(id)

    async def get_or_404(self, id: int) -> Departement:
        ent = await self._repo.find_by_id(id)
        if ent is None:
            self._raise_not_found(Messages.DEPARTEMENT_NOT_FOUND)
        return ent
    ...
    async def create(self, data: DepartementCreate) -> Departement:
        if await self._entreprise_repo.find_by_id(data.entreprise_id) is None:
            self._raise_not_found(Messages.ENTREPRISE_NOT_FOUND)
        code = (data.code or "").strip()
        if not code:
            self._raise_bad_request(Messages.DEPARTEMENT_CODE_VIDE)
        if await self._repo.exists_by_entreprise_and_code(data.entreprise_id, code):
            self._raise_conflict(Messages.DEPARTEMENT_CODE_EXISTS.format(code=code))
        ent = Departement(
    ...
    async def update(self, id: int, data: DepartementUpdate) -> Departement:
        ...
        if "code" in update_data and update_data["code"] is not None:
            ...
            if await self._repo.exists_by_entreprise_and_code(ent.entreprise_id, update_data["code"], exclude_id=id):
                self._raise_conflict(Messages.DEPARTEMENT_CODE_EXISTS.format(code=update_data["code"]))

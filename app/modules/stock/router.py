...
# --- Stocks par dépôt ---
@router.get("/depots/{depot_id}/stocks", response_model=list[schemas.StockResponse])
...
@router.get("/depots/{depot_id}/stocks/{stock_id}", response_model=schemas.StockResponse)
...
# --- Stocks par produit ---
@router.get("/produits/{produit_id}/stocks", response_model=list[schemas.StockResponse])
...
# --- Mouvements ---
@router.get("/mouvements", response_model=list[schemas.MouvementStockResponse])
...
@router.get("/mouvements/{id}", response_model=schemas.MouvementStockResponse)
async def get_mouvement(db: DbSession, current_user: CurrentUser, id: int):
    return await MouvementService(db).get_or_404(id)


@router.post("/mouvements", response_model=schemas.MouvementStockResponse, status_code=201)
...
# --- Alertes ---
@router.get("/alertes", response_model=list[schemas.AlerteStockResponse])

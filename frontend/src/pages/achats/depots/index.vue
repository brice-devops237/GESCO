...
import { listDepots, deleteDepot } from '@/api/achats'
import type { DepotResponse } from '@/api/types/achats'
import DepotFormModal from '@/components/achats/DepotFormModal.vue'
...
const headers = [
  ...
  { title: 'ADRESSE', key: 'adresse', width: '180px' },
  { title: 'VILLE', key: 'ville', width: '120px' },
  { title: 'PAYS', key: 'pays', width: '80px' },
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(d => (d.code?.toLowerCase().includes(q)) || (d.libelle?.toLowerCase().includes(q)) || (d.ville?.toLowerCase().includes(q)))
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listDepots({ entreprise_id: eid, limit: 200 })
  } catch (err) {
...
function resetFilters() { searchQuery.value = ''; page.value = 1; load() }
onMounted(load)
...
function openEdit(row: DepotResponse) { editingId.value = row.id; formModalOpen.value = true }
async function openConfirmDelete(row: DepotResponse) {
  const result = await Swal.fire({
    title: 'Êtes-vous sûr ?',
    html: `Supprimer le dépôt <strong>« ${row.code} - ${row.libelle} »</strong> ?`,
  ...
  try {
    ...
    toastStore.success('Dépôt supprimé.')
    await load()
  } catch (err) {
...
<template>
  <div class="achats-page">
    <VRow v-if="entrepriseId == null">
      <VCol cols="12"><VAlert type="info" variant="tonal" class="rounded-lg">Aucune entreprise associée. Les dépôts sont rattachés à une entreprise.</VAlert></VCol>
    </VRow>
    <VCard v-else class="achats-card">
      <VCardText class="pa-0">
        <div class="filters-bar d-flex flex-wrap align-center gap-3 pa-5 pb-4">
          ...
          <VBtn color="primary" prepend-icon="ri-add-line" @click="openCreate">Ajouter un dépôt</VBtn>
        </div>
        <VDivider />
        <div class="achats-table-wrap">
          <VTable class="achats-table" :class="{ 'table-loading': loading }">
            ...
            <tbody>
              ...
              <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun dépôt.</td></tr>
              <tr v-else v-for="item in paginatedItems" :key="item.id" class="achats-row">
                ...
                <td class="py-3 px-4 text-body-2">{{ item.adresse || '—' }}</td>
    ...
    <DepotFormModal v-if="entrepriseId" v-model="formModalOpen" :entreprise-id="entrepriseId" :depot-id="editingId" @saved="onFormSaved" @cancel="onFormCancel" />
  </div>
</template>
...
.achats-table-wrap { overflow: hidden; }
.achats-table { border-collapse: collapse; }
.achats-table :deep(thead th) { background-color: rgba(var(--v-theme-on-surface), 0.04); }
.achats-table th { white-space: nowrap; }

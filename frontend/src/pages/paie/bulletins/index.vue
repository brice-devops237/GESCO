...
import { listBulletinsPaie } from '@/api/paie'
import type { BulletinPaieResponse } from '@/api/types/paie'
...
const items = ref<BulletinPaieResponse[]>([])
const loading = ref(false)

const headers = [
  { title: 'ID', key: 'id', width: '80px' },
  { title: 'EMPLOYÉ ID', key: 'employe_id', width: '110px' },
  { title: 'PÉRIODE PAIE ID', key: 'periode_paie_id', width: '130px' },
  { title: 'NET À PAYER', key: 'net_a_payer', width: '120px', align: 'end' as const },
]
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listBulletinsPaie({ entreprise_id: entrepriseId, limit: 300 })
  } catch (err) {
  ...
  <div class="module-page">
    <VCard>
      <VCardText class="pa-0">
        <div class="pa-5 pb-4"><h3 class="text-h6">Bulletins de paie</h3></div>
        <VDivider />
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun bulletin.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              <td class="py-3 px-4">{{ item.id }}</td>
              <td class="py-3 px-4">{{ item.employe_id }}</td>
              <td class="py-3 px-4">{{ item.periode_paie_id }}</td>
              <td class="py-3 px-4 text-right">{{ item.net_a_payer ?? '—' }}</td>
            </tr>
          </tbody>
        </VTable>
        <VDivider />

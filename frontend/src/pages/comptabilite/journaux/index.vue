...
import { listJournauxComptables } from '@/api/comptabilite'
import type { JournalComptableResponse } from '@/api/types/comptabilite'
import JournalComptableFormModal from '@/components/comptabilite/JournalComptableFormModal.vue'
...
const items = ref<JournalComptableResponse[]>([])
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(j => (j.code?.toLowerCase().includes(q)) || (j.libelle?.toLowerCase().includes(q)))
  if (filterStatut.value === 'actif') list = list.filter(j => j.actif)
  if (filterStatut.value === 'inactif') list = list.filter(j => !j.actif)
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listJournauxComptables({ entreprise_id: entrepriseId, limit: 200, actif_only: filterStatut.value === 'actif' ? true : undefined })
  } catch (err) {
...
function openEdit(row: JournalComptableResponse) { editingId.value = row.id; formModalOpen.value = true }
...
<template>
  <div class="module-page">
    <VCard>
      <VCardText class="pa-0">
        <div class="d-flex flex-wrap align-center gap-3 pa-5 pb-4">
          <VTextField v-model="searchQuery" placeholder="Rechercher (code, libellé…)" density="compact" hide-details clearable style="min-width: 220px;" prepend-inner-icon="ri-search-line" variant="outlined" />
        ...
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun journal trouvé.</td></tr>
    ...
    <JournalComptableFormModal v-model="formModalOpen" :journal-id="editingId" @saved="onFormSaved" @cancel="onFormCancel" />
  </div>
</template>

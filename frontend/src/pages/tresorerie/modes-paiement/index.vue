...
import { listModesPaiement } from '@/api/tresorerie'
import type { ModePaiementResponse } from '@/api/types/tresorerie'
import ModePaiementFormModal from '@/components/tresorerie/ModePaiementFormModal.vue'
...
const items = ref<ModePaiementResponse[]>([])
...
const headers = [
  ...
  { title: 'CODE OPÉRATEUR', key: 'code_operateur', width: '120px' },
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(m => (m.code?.toLowerCase().includes(q)) || (m.libelle?.toLowerCase().includes(q)) || (m.code_operateur?.toLowerCase().includes(q)))
  if (filterStatut.value === 'actif') list = list.filter(m => m.actif)
  if (filterStatut.value === 'inactif') list = list.filter(m => !m.actif)
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listModesPaiement({
...
function openEdit(row: ModePaiementResponse) {
        ...
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              ...
              <td class="py-3 px-4 text-body-2">{{ item.code_operateur || '—' }}</td>

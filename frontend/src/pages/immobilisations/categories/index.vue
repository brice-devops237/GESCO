...
import { listCategoriesImmobilisations } from '@/api/immobilisations'
import type { CategorieImmobilisationResponse } from '@/api/types/immobilisations'
...
const items = ref<CategorieImmobilisationResponse[]>([])
...
const headers = [
  ...
  { title: 'DURÉE AMORT. (ans)', key: 'duree_amortissement_annees', width: '150px' },
  { title: 'TAUX', key: 'taux_amortissement', width: '100px' },
]
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listCategoriesImmobilisations({ entreprise_id: entrepriseId, limit: 200 })
  } catch (err) {
      ...
      <VCardText class="pa-0">
        ...
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucune catégorie.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              ...
              <td class="py-3 px-4">{{ item.duree_amortissement_annees ?? '—' }}</td>
              <td class="py-3 px-4">{{ item.taux_amortissement ?? '—' }}</td>
            </tr>
          </tbody>
        </VTable>
        <VDivider />

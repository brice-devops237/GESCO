...
import { listImmobilisations } from '@/api/immobilisations'
import type { ImmobilisationResponse } from '@/api/types/immobilisations'
...
const items = ref<ImmobilisationResponse[]>([])
...
const headers = [
  ...
  { title: 'DATE ACQUISITION', key: 'date_acquisition', width: '140px' },
  { title: 'VALEUR', key: 'valeur_acquisition', width: '120px', align: 'end' as const },
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(i => (i.code?.toLowerCase().includes(q)) || (i.designation?.toLowerCase().includes(q)))
  if (filterStatut.value === 'actif') list = list.filter(i => i.actif)
  if (filterStatut.value === 'inactif') list = list.filter(i => !i.actif)
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listImmobilisations({ entreprise_id: entrepriseId, limit: 300, actif_only: filterStatut.value === 'actif' ? true : undefined })
  } catch (err) {
    ...
    <VCard>
      <VCardText class="pa-0">
        <div class="d-flex flex-wrap align-center gap-3 pa-5 pb-4">
          <VTextField v-model="searchQuery" placeholder="Rechercher (code, désignation)…" density="compact" hide-details clearable style="min-width: 220px;" prepend-inner-icon="ri-search-line" variant="outlined" />
        ...
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun actif.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              ...
              <td class="py-3 px-4">{{ item.date_acquisition }}</td>
              <td class="py-3 px-4 text-right">{{ item.valeur_acquisition ?? '—' }}</td>

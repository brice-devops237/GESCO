...
import { listPostes } from '@/api/rh'
import type { PosteResponse } from '@/api/types/rh'
...
const headers = [
  ...
  { title: 'DÉPARTEMENT ID', key: 'departement_id', width: '120px' },
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listPostes({ entreprise_id: entrepriseId, limit: 300, actif_only: filterStatut.value === 'actif' ? true : undefined })
  } catch (err) {
      ...
      <VCardText class="pa-0">
        ...
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun poste.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              ...
              <td class="py-3 px-4">{{ item.departement_id ?? '—' }}</td>

...
import { listEmployes } from '@/api/rh'
import type { EmployeResponse } from '@/api/types/rh'
...
const items = ref<EmployeResponse[]>([])
const loading = ref(false)
const searchQuery = ref('')
const filterStatut = ref<'all' | 'actif' | 'inactif'>('all')

const headers = [
  { title: 'MATRICULE', key: 'matricule', width: '120px' },
  { title: 'NOM', key: 'nom' },
  { title: 'PRÉNOM', key: 'prenom' },
  { title: 'POSTE ID', key: 'poste_id', width: '90px' },
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(e => (e.matricule?.toLowerCase().includes(q)) || (e.nom?.toLowerCase().includes(q)) || (e.prenom?.toLowerCase().includes(q)))
  if (filterStatut.value === 'actif') list = list.filter(e => e.actif)
  if (filterStatut.value === 'inactif') list = list.filter(e => !e.actif)
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listEmployes({ entreprise_id: entrepriseId, limit: 300, actif_only: filterStatut.value === 'actif' ? true : undefined })
  } catch (err) {
    ...
    <VCard>
      <VCardText class="pa-0">
        <div class="d-flex flex-wrap align-center gap-3 pa-5 pb-4">
          <VTextField v-model="searchQuery" placeholder="Rechercher (matricule, nom, prénom)…" density="compact" hide-details clearable style="min-width: 240px;" prepend-inner-icon="ri-search-line" variant="outlined" />
        ...
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun employé.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              <td class="py-3 px-4 font-weight-medium">{{ item.matricule || '—' }}</td>
              <td class="py-3 px-4">{{ item.nom || '—' }}</td>
              <td class="py-3 px-4">{{ item.prenom || '—' }}</td>
              <td class="py-3 px-4">{{ item.poste_id ?? '—' }}</td>

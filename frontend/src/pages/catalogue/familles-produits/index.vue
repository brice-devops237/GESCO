...
import { listFamillesProduits, deleteFamilleProduit } from '@/api/catalogue'
import type { FamilleProduitResponse } from '@/api/types/catalogue'
import FamilleProduitFormModal from '@/components/catalogue/FamilleProduitFormModal.vue'
...
const items = ref<FamilleProduitResponse[]>([])
...
const headers = [
  ...
  { title: 'NIVEAU', key: 'niveau', width: '90px' },
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(f => (f.code?.toLowerCase().includes(q)) || (f.libelle?.toLowerCase().includes(q)))
  if (filterStatut.value === 'actif') list = list.filter(f => f.actif)
  if (filterStatut.value === 'inactif') list = list.filter(f => !f.actif)
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listFamillesProduits({ entreprise_id: eid, limit: 200, actif_only: filterStatut.value === 'actif' ? true : undefined, search: searchQuery.value?.trim() || undefined })
  } catch (err) {
...
function openEdit(row: FamilleProduitResponse) { editingId.value = row.id; formModalOpen.value = true }
async function openConfirmDelete(row: FamilleProduitResponse) {
  const result = await Swal.fire({
    title: 'Êtes-vous sûr ?',
    html: `Supprimer la famille <strong>« ${row.code } - ${row.libelle } »</strong> ?`,
  ...
  try {
    await deleteFamilleProduit(row.id)
    toastStore.success('Famille supprimée.')
    await load()
  } catch (err) {
...
<template>
  <div class="catalogue-page">
    <VRow v-if="entrepriseId == null">
      <VCol cols="12"><VAlert type="info" variant="tonal" class="rounded-lg">Aucune entreprise associée. Les familles sont rattachées à une entreprise.</VAlert></VCol>
    </VRow>
        ...
        <div class="catalogue-table-wrap">
          <VTable class="catalogue-table" :class="{ 'table-loading': loading }">
            ...
            <tbody>
              ...
              <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucune famille trouvée.</td></tr>
              <tr v-else v-for="item in paginatedItems" :key="item.id" class="catalogue-row">
                ...
                <td class="py-3 px-4 text-body-2">{{ item.niveau }}</td>
    ...
    <FamilleProduitFormModal v-if="entrepriseId" v-model="formModalOpen" :entreprise-id="entrepriseId" :famille-id="editingId" @saved="onFormSaved" @cancel="onFormCancel" />
  </div>
</template>

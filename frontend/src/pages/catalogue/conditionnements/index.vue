...
import { listConditionnements, listUnitesMesure, deleteConditionnement } from '@/api/catalogue'
import type { ConditionnementResponse } from '@/api/types/catalogue'
import type { UniteMesureResponse } from '@/api/types/catalogue'
import ConditionnementFormModal from '@/components/catalogue/ConditionnementFormModal.vue'
...
const items = ref<ConditionnementResponse[]>([])
const unites = ref<UniteMesureResponse[]>([])
const uniteMap = computed(() => {
  const m: Record<number, string> = {}
  for (const u of unites.value) m[u.id] = u.code
  return m
})
function uniteCode(uniteId: number) { return uniteMap.value[uniteId] ?? String(uniteId) }
...
const headers = [
  ...
  { title: 'QUANTITÉ', key: 'quantite_unites', width: '100px', align: 'end' as const },
  { title: 'UNITÉ', key: 'unite_id', width: '90px' },
...
async function loadUnites() {
  try { unites.value = await listUnitesMesure({ limit: 200 }) }
  catch { unites.value = [] }
}
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listConditionnements({ entreprise_id: eid, limit: 200, actif_only: filterStatut.value === 'actif' ? true : undefined, search: searchQuery.value?.trim() || undefined })
  } catch (err) {
...
function openEdit(row: ConditionnementResponse) { editingId.value = row.id; formModalOpen.value = true }
async function openConfirmDelete(row: ConditionnementResponse) {
  const result = await Swal.fire({
    title: 'Êtes-vous sûr ?',
    html: `Supprimer le conditionnement <strong>« ${row.code} - ${row.libelle || ''} »</strong> ?`,
  ...
  try {
    await deleteConditionnement(row.id)
    toastStore.success('Conditionnement supprimé.')
    await load()
  } catch (err) {
...
<template>
  <div class="catalogue-page">
    <VRow v-if="entrepriseId == null">
      <VCol cols="12"><VAlert type="info" variant="tonal" class="rounded-lg">Aucune entreprise associée. Les conditionnements sont rattachés à une entreprise.</VAlert></VCol>
    </VRow>
        ...
        <div class="catalogue-table-wrap">
          <VTable class="catalogue-table" :class="{ 'table-loading': loading }">
            ...
            <tbody>
              ...
              <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun conditionnement trouvé.</td></tr>
              <tr v-else v-for="item in paginatedItems" :key="item.id" class="catalogue-row">
                ...
                <td class="py-3 px-4 text-right">{{ item.quantite_unites }}</td>
                <td class="py-3 px-4 text-body-2">{{ uniteCode(item.unite_id) }}</td>
    ...
    <ConditionnementFormModal v-if="entrepriseId" v-model="formModalOpen" :entreprise-id="entrepriseId" :conditionnement-id="editingId" @saved="onFormSaved" @cancel="onFormCancel" />
  </div>
</template>

...
import { listTauxTva, deleteTauxTva } from '@/api/catalogue'
import type { TauxTvaResponse } from '@/api/types/catalogue'
import TauxTvaFormModal from '@/components/catalogue/TauxTvaFormModal.vue'
import { useToastStore } from '@/stores/toast'
import { getApiErrorMessage } from '@/utils/apiErrors'
import Swal from 'sweetalert2'

const toastStore = useToastStore()
const items = ref<TauxTvaResponse[]>([])
...
const headers = [
  ...
  { title: 'TAUX %', key: 'taux', width: '100px', align: 'end' as const },
  { title: 'LIBELLÉ', key: 'libelle', sortable: true },
  { title: 'NATURE', key: 'nature', width: '110px' },
...
async function load() {
  loading.value = true
  try {
    items.value = await listTauxTva({ limit: 200, actif_only: filterStatut.value === 'actif' ? true : undefined })
  } catch (err) {
...
function openEdit(row: TauxTvaResponse) { editingId.value = row.id; formModalOpen.value = true }
async function openConfirmDelete(row: TauxTvaResponse) {
  const result = await Swal.fire({
    title: 'Êtes-vous sûr ?',
    html: `Supprimer le taux TVA <strong>« ${row.code} - ${row.libelle || ''} »</strong> ?`,
  ...
  try {
    await deleteTauxTva(row.id)
    toastStore.success('Taux TVA supprimé.')
    await load()
  } catch (err) {
...
<template>
  <div class="catalogue-page">
        ...
        <div class="catalogue-table-wrap">
          <VTable class="catalogue-table" :class="{ 'table-loading': loading }">
            ...
            <tbody>
              ...
              <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun taux TVA trouvé.</td></tr>
              <tr v-else v-for="item in paginatedItems" :key="item.id" class="catalogue-row">
                ...
                <td class="py-3 px-4 text-right font-weight-medium">{{ item.taux }} %</td>
                <td class="py-3 px-4 text-body-2">{{ item.libelle || '—' }}</td>
                <td class="py-3 px-4 text-body-2">{{ item.nature || '—' }}</td>
    ...
    <TauxTvaFormModal v-model="formModalOpen" :taux-id="editingId" @saved="onFormSaved" @cancel="onFormCancel" />
  </div>
</template>

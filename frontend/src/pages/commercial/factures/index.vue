...
import { listFactures } from '@/api/commercial'
import type { FactureResponse } from '@/api/types/commercial'
...
const items = ref<FactureResponse[]>([])
...
const headers = [
  ...
  { title: 'DATE', key: 'date_facture', width: '120px' },
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(f => (f.numero?.toLowerCase().includes(q)) || String(f.client_id).includes(q))
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listFactures({ entreprise_id: entrepriseId, limit: 300 })
  } catch (err) {
      ...
      <VCardText class="pa-0">
        ...
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucune facture.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              ...
              <td class="py-3 px-4">{{ item.date_facture }}</td>

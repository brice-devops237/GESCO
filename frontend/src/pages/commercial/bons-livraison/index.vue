...
import { listBonsLivraison } from '@/api/commercial'
import type { BonLivraisonResponse } from '@/api/types/commercial'
...
const items = ref<BonLivraisonResponse[]>([])
...
const headers = [
  ...
  { title: 'DATE LIVRAISON', key: 'date_livraison', width: '130px' },
]
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(b => (b.numero?.toLowerCase().includes(q)) || String(b.client_id).includes(q))
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listBonsLivraison({ entreprise_id: entrepriseId, limit: 300 })
  } catch (err) {
    ...
    <VCard>
      <VCardText class="pa-0">
        <div class="d-flex flex-wrap align-center gap-3 pa-5 pb-4">
          <VTextField v-model="searchQuery" placeholder="Rechercher (n°, commande)…" density="compact" hide-details clearable style="min-width: 200px;" prepend-inner-icon="ri-search-line" variant="outlined" />
        </div>
        <VDivider />
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun bon de livraison.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              ...
              <td class="py-3 px-4">{{ item.date_livraison }}</td>
            </tr>
          </tbody>
        </VTable>
        <VDivider />

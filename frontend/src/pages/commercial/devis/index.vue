...
import { listDevis } from '@/api/commercial'
import type { DevisResponse } from '@/api/types/commercial'
...
const headers = [
  ...
  { title: 'DATE', key: 'date_devis', width: '120px' },
  { title: 'ÉTAT ID', key: 'etat_id', width: '90px' },
...
const filteredItems = computed(() => {
  ...
  if (q) list = list.filter(d => (d.numero?.toLowerCase().includes(q)) || String(d.client_id).includes(q))
  return list
})
...
async function load() {
  ...
  loading.value = true
  try {
    items.value = await listDevis({ entreprise_id: entrepriseId, limit: 300 })
  } catch (err) {
    ...
    <VCard>
      <VCardText class="pa-0">
        <div class="d-flex flex-wrap align-center gap-3 pa-5 pb-4">
          <VTextField v-model="searchQuery" placeholder="Rechercher (n°, client)…" density="compact" hide-details clearable style="min-width: 200px;" prepend-inner-icon="ri-search-line" variant="outlined" />
        </div>
        <VDivider />
        <VTable :class="{ 'table-loading': loading }">
          ...
          <tbody>
            ...
            <tr v-else-if="paginatedItems.length === 0"><td :colspan="headers.length" class="text-center py-8 text-medium-emphasis">Aucun devis.</td></tr>
            <tr v-else v-for="item in paginatedItems" :key="item.id" class="data-row">
              <td class="py-3 px-4 font-weight-medium">{{ item.numero }}</td>
              <td class="py-3 px-4">{{ item.client_id }}</td>
              <td class="py-3 px-4">{{ item.date_devis }}</td>
              <td class="py-3 px-4">{{ item.etat_id }}</td>
              <td class="py-3 px-4 text-right">{{ item.montant_ttc ?? '—' }}</td>
            </tr>
          </tbody>
        </VTable>
        <VDivider />
